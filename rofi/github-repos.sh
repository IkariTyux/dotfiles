...
clone_repository(){
  ...
  if [ -z "$repository" ]; then
    ...
    git clone "$URL$repository" /home/shinji/Code/github/$repository
  fi
}
...
all_my_repositories_short_name(){
	curl -s "https://api.github.com/users/$USER/repos?per_page=1000" | grep -o 'git@[^*]*' | sed "s/git@github.com:$USER\///g;s/.git\"\,//g"
 }

# Rofi dmenu
rofi_dmenu(){
  rofi -dmenu -matching fuzzy -no-custom -p "Github Repositories "\
